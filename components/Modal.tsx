...
const Modal: FC<{ close: () => void }> = ({ close }) => {
  ...
  return (
      <section className="absolute flex flex-1 justify-center items-center bg-gray-100/50 inset-0 z-20">
          <div className="bg-medium w-3/4 h-3/4 rounded-lg shadow-lg shadow-medium/30 flex flex-col justify-center gap-12 items-center relative">
              <Cancel onClick={()=> close()} className="w-6 h-6 absolute top-6 right-6 cursor-pointer"/>
          <select ref={state} className="py-3 px-6 outline-none  ring-2 ring-light text-light bg-transparent rounded-md">
