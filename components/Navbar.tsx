...
const Navbar: FC = () => {
  ...
  return (
    <nav>
      <aside className={`${`${isDesktop ? `w-24 group fixed  left-0 hover:w-56` : `fixed ${!isMobile ? `w-0 -left-full` : `w-24 h-screen inset-y-0 left-0`} `}`} cursor-pointer transition-all duration-300 ease-in overflow-x-hidden  h-full  left-0 top-0 bottom-0 z-10 flex bg-neutral justify-center gap-24 flex-col  w-full items-center content-center`}>
          <Link href="/" className="mx-auto">
            <div className={`cursor-pointer ${isDesktop ? `gap-4 group-hover:flex` : `block`} `} onClick={() => setIsMobile (false)}>
              <span className="bg-light w-12 h-12 rounded-full flex items-center justify-center">
      ...
      <div className={`bg-transparent relative top-4 lg:hidden ${!isMobile ? `left-4` : `left-56`} transition-all duration-500 ease-in`} onClick={navHandler} onBlur={() => setIsMobile(false)} tabIndex={24}>
          <span className="w-14 h-1 cursor-pointer rounded-full  bg-neutral block after:content-[''] after:relative before:w-12 after:h-1 after:bg-neutral after:block after:mt-1 after:float-right after:ml-auto before:content-[''] before:relative after:w-10 before:h-1 before:mt-2 before:float-right before:ml-auto before:bg-neutral before:block self-end before:self-end after:self-end before:rounded-full after:rounded-full lg:-z-20"></span>
        </div>
    </nav>
  );
};
