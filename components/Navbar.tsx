...
const Navbar: FC = () => {
  ...
  return (
    <nav>
      <aside className={`${`${isDesktop ? `w-24 group fixed  left-0 hover:w-56` : `fixed ${!isMobile ? `w-0 -left-full` : `w-24 h-screen inset-y-0 left-0`} `}`} cursor-pointer transition-all duration-300 ease-in overflow-x-hidden  h-full  left-0 top-0 bottom-0 z-10 flex bg-[#429EBD] justify-center gap-24 flex-col  w-full items-center content-center`}>
